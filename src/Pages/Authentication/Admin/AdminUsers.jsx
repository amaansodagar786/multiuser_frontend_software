...
const AdminUsers = () => {
  ...
  const isCurrentUserAdmin = useMemo(() => {
    return currentUser.permissions?.includes('admin') || false;
  }, [currentUser]);
  ...
  // Available permissions (Super Admin not included - it's database only)
  ...
  const filteredUsers = useMemo(() => {
    ...
    return baseUsers;
  }, [debouncedSearch, users, isCurrentUserAdmin, isCurrentUserSuperAdmin]);
  ...
  const UserModal = ({ user, onClose, onExport, onUpdate, onDelete }) => {
    ...
    // Check if editing user is admin
    const isEditingUserAdmin = useMemo(() => {
      return user?.permissions?.includes('admin') || false;
    }, [user]);

    // Check if editing user is superadmin
    const isEditingUserSuperAdmin = useMemo(() => {
      return user?.permissions?.includes('superadmin') || false;
    }, [user]);

    // Determine if password can be updated
    const canUpdatePassword = useMemo(() => {
      // Super Admin can update anyone's password
      if (isCurrentUserSuperAdmin) return true;
      
      // Admin can update passwords for non-admin users only
      if (isCurrentUserAdmin && !isEditingUserAdmin && !isEditingUserSuperAdmin) return true;
      
      return false;
    }, [isCurrentUserSuperAdmin, isCurrentUserAdmin, isEditingUserAdmin, isEditingUserSuperAdmin]);
        ...
        <div className="modal-content" onClick={e => e.stopPropagation()}>
          <div className="modal-header">
            <div className="modal-title">
              {isEditingUserSuperAdmin && <FaCrown className="superadmin-crown" />}
              {isEditing ? "Edit User" : `User Details: ${user.name}`}
              {isEditingUserSuperAdmin && <span className="superadmin-badge">Super Admin</span>}
            </div>
          ...
          <div className="modal-body">
            <div className="wo-details-grid">
              ...
              <div className="detail-row">
                ...
                {isEditing ? (
                      ...
                      {availablePermissions.map(permission => (
                        <label key={permission.id} className="permission-checkbox-horizontal">
                          <input
                            ...
                            disabled={isEditingUserSuperAdmin} // Cannot change Super Admin permissions
                          />
                  ...
                  <span className="detail-value">
                    ...
                    {isEditingUserSuperAdmin && <span className="superadmin-badge-inline"> (Super Admin)</span>}
                  </span>
                )}
              </div>

              {/* Password Update Section */}
              {isEditing && canUpdatePassword && (
                <div className="detail-row">
              ...
              {/* Password update restriction message */}
              {isEditing && !canUpdatePassword && (
                <div className="detail-row">
                  <div className="admin-note">
                    <FaExclamationTriangle />
                    {isEditingUserSuperAdmin 
                      ? "Only Super Admin can update passwords for Admin users."
                      : "Password update is not available for this user."
                    }
                  </div>
                </div>
              )}
          ...
          <div className="modal-footer">
            ...
            <button
              ...
              disabled={isEditingUserSuperAdmin && !isCurrentUserSuperAdmin} // Only Super Admin can edit Super Admin
            >
              {isEditing ? <FaSave /> : <FaEdit />}
              {isEditing ? "Save Changes" : "Update"}
              {isEditingUserSuperAdmin && !isCurrentUserSuperAdmin && " (Restricted)"}
            </button>
            <button
              className="delete-btn"
              onClick={() => setShowDeleteConfirm(true)}
              disabled={isEditingUserSuperAdmin} // Cannot delete Super Admin
            >
              <FaTrash /> 
              {isEditingUserSuperAdmin ? "Cannot Delete" : "Delete"}
            </button>
          </div>
        </div>
      ...
      <div className="main">
        ...
        <div className="section-tabs">
          ...
          >
            ...
            {isCurrentUserSuperAdmin && <FaCrown className="superadmin-indicator" />}
          </button>
          <button
        ...
        {activeSection === "users" && (
          <>
            ...
            <div className="current-user-info">
              ...
              <span className={`user-role ${isCurrentUserSuperAdmin ? 'superadmin' : isCurrentUserAdmin ? 'admin' : 'user'}`}>
                {isCurrentUserSuperAdmin ? 'Super Admin' : isCurrentUserAdmin ? 'Admin' : 'User'}
                {isCurrentUserSuperAdmin && <FaCrown />}
              </span>
            </div>
                  ...
                  <table>
                    ...
                    <tbody>
                      {paginatedUsers.map((user, index) => {
                        const isUserSuperAdmin = user.permissions?.includes('superadmin');
                        const isUserAdmin = user.permissions?.includes('admin');
                        ...
                        return (
                          ...
                          >
                            <td>
                              {user.name}
                              {isUserSuperAdmin && <FaCrown className="superadmin-crown-table" title="Super Admin" />}
                              {isUserAdmin && !isUserSuperAdmin && <span className="admin-badge">Admin</span>}
                            </td>
                            <td>{user.email}</td>
                            <td>{user.phone}</td>
                            <td>
                              {user.permissions ? user.permissions.join(', ') : 'No permissions'}
                              {isUserSuperAdmin && <span className="superadmin-badge-inline"> (Super Admin)</span>}
                            </td>
                          </tr>
                        );
                      })}
